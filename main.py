...
import testing.test_agents_off as testing
import ast
...
def main():
    ...
    parser.add_argument('--pretrained_agents',help='Set to True if the agents have been pretrained', action='store_true',default=False)
    ...
    parser.add_argument('--only_test',help='Set to True if agents will only be tested', action='store_true',default=False)
    ...
    if args['pretrained_agents'] or args['only_test']:
        pretrained_weights = np.load('{}/{}/pretrained_weights.npy'.format(args['summary_dir'],args['algo']), allow_pickle=True)
        s_desired = np.load('{}/{}/desired_state.npy'.format(args['summary_dir'],args['algo']), allow_pickle=True)
    ...
    for node in range(args['n_agents']):
        ...
        if args['pretrained_agents'] or args['only_test']:
            ...
            if args['algo']=='GenPBE' or args['algo'] == "saddle_point":
                bellman.layers[-1].set_weights(pretrained_weights[node][3][-2:])
    ...
    if args['only_test'] == False:
        ...
        #----------------------------------------------------------------------------------------------------
        sim_data.to_pickle("{}/{}/sim_train_data.pkl".format(args['summary_dir'],args['algo']))
        np.save('{}/{}/pretrained_weights.npy'.format(args['summary_dir'],args['algo']), agent_weights, allow_pickle=True)
        np.save('{}/{}/desired_state.npy'.format(args['summary_dir'],args['algo']),s_desired,allow_pickle=True)

    #---------------------------------------------------------------------------------------------------------------------------------------------
    '''TEST AGENTS'''
    agent_weights,sim_data = testing.test_MARL(env,agents,args,test_summary_writer)
    sim_data.to_pickle("{}/{}/sim_test_data.pkl".format(args['summary_dir'],args['algo']))
